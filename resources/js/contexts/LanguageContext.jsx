...
export function LanguageProvider({ children }) {
    ...
    return (
        <LanguageContext.Provider value={{ lang, setLang, changeLanguage, isLoading, t: translations[lang] }}>
            <LanguageLoader isLoading={isLoading} />
            {children}
...
function LanguageLoader({ isLoading }) {
    return (
        <div
        ...
        >
            <div className={`flex flex-col items-center justify-center transform transition-all duration-700 ${isLoading ? "scale-100 opacity-100" : "scale-125 opacity-0"}`}>
                <div className="relative w-20 h-20 flex items-center justify-center">
                    {/* Outer Rotating Ring */}
                    <div className="absolute inset-0 rounded-full border-2 border-emerald-100 border-t-emerald-800 animate-spin"></div>

                    {/* Inner Pulse Icon */}
                    <div className="w-12 h-12 rounded-xl bg-emerald-800 flex items-center justify-center text-white shadow-lg animate-pulse z-10">
                ...
                <div className="mt-6 text-center">
                    ...
                    <div className="flex justify-center gap-1 mt-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-800 animate-bounce delay-0"></div>
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-800 animate-bounce delay-150"></div>
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-800 animate-bounce delay-300"></div>
                    </div>
                </div>
            </div>
        </div>
...
export function LanguageSwitcher() {
    ...
    return (
        <div className="flex items-center bg-emerald-50 rounded-full p-0.5 border border-emerald-200">
            {langs.map((l) => (
                <button
                    key={l.code}
                    onClick={() => changeLanguage(l.code)}
                    className={`px-2.5 py-1 text-xs rounded-full font-medium transition ${lang === l.code
