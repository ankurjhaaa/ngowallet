...
export default function UserDetail() {
	...
	const openReceiptFromPayment = (payment) => {
		const normalized = {
			...
			receipt_pdf_url: payment.receipt_pdf_url || payment.receipt_url,
			receipt_download_url: payment.receipt_download_url || payment.receipt_url,
	...
	return (
		<AdminLayout title="User Dashboard">
			<div className="w-full min-w-0 overflow-x-hidden px-1 pb-20 mt-2">
				...
				{/* ================= SUMMARY STATS ================= */}
				<div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
					<div className="bg-red-800 rounded-md p-4 text-white relative overflow-hidden group">
						<div className="absolute -right-2 -top-2 w-16 h-16 bg-white/5 rounded-md blur-xl group-hover:scale-150 transition-transform duration-500"></div>
						...
						<div className="mt-2 flex items-center gap-2 text-red-200 text-[9px] font-medium">
							<span className="w-4 h-4 rounded-md bg-red-700 flex items-center justify-center text-[8px]">
					...
					<div className="bg-white rounded-md p-4 border border-gray-100 relative overflow-hidden group">
						<div className="absolute -right-2 -top-2 w-16 h-16 bg-gray-50 rounded-md blur-xl group-hover:scale-150 transition-transform duration-500"></div>
						...
						<div className="mt-2 flex items-center gap-2 text-[9px] font-medium">
							{totalDeu > 0 ? (
								<div className="flex items-center gap-2 text-red-400">
									<span className="w-4 h-4 rounded-md bg-red-50 flex items-center justify-center text-red-600 text-[8px]">
								...
								<div className="flex items-center gap-2 text-emerald-600">
									<span className="w-4 h-4 rounded-md bg-emerald-50 flex items-center justify-center text-emerald-600 text-[8px]">
										<i className="fas fa-check"></i>
									</span>
									Settled
								</div>
				...
				<div className="bg-white rounded-md border border-gray-100 mb-4 overflow-hidden transition-all duration-300">
					<button
					...
					>
						<div className="flex items-center gap-3">
							<div className="w-8 h-8 rounded-md bg-red-100 text-red-800 flex items-center justify-center font-bold text-xs shrink-0">
				...
				<div className="mb-4">
					<div className="flex items-center justify-between mb-4">
						...
						<button
							...
							className="h-7 px-3 rounded-md bg-red-800 text-white text-[9px] font-bold hover:bg-red-700 transition cursor-pointer"
						>
							Add Plan
						</button>
					</div>
					...
					{user_plans && user_plans.length > 0 ? (
							...
							<div className="w-full max-w-full overflow-x-auto overflow-y-hidden overscroll-x-contain pb-1 [scrollbar-width:none] [-ms-overflow-style:none] touch-pan-x">
								<div className="grid grid-flow-col auto-cols-[85vw] sm:auto-cols-[300px] gap-3 min-w-max px-1 pb-3 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden">
									{user_plans.map((up) => (
										<div
											key={up.id}
											className={`w-full max-w-[300px] bg-white rounded-md p-4 border transition snap-start ${up.status === 'completed' ? 'border-emerald-100' : 'border-red-50'
												}`}
										>
											<div className="flex items-center justify-between mb-3">
												<div className="flex items-center gap-2 overflow-hidden">
													<div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${up.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-700'
														}`}>
												...
												<div className="flex items-center gap-2">
													<a
														...
														className="h-7 w-7 rounded-md border border-gray-100 bg-white text-red-700 flex items-center justify-center hover:bg-red-50 cursor-pointer"
														title="Download PDF"
													>
											...
											<div className="mb-4">
												...
												<div className="w-full bg-gray-100 h-1 rounded-md overflow-hidden">
													<div
											...
											{openPayments[up.id] && (
												<div className="mt-3 bg-gray-50/50 rounded-md p-2 border border-gray-100 animate-in fade-in slide-in-bg-top-1 duration-200">
													{up.payments && up.payments.length > 0 ? (
														<div className="space-y-1.5 max-h-[100px] overflow-y-auto pr-1 text-[9px]">
															{up.payments.map((p, i) => (
																<div key={i} className="flex items-center justify-between gap-2 bg-white px-2 py-1 rounded-md border border-gray-50">
						...
						<div className="bg-white rounded-md p-4 border border-dashed border-gray-200 text-center">
							<div className="w-10 h-10 bg-gray-50 rounded-md flex items-center justify-center text-gray-400 mx-auto mb-3">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-10 gap-3">
							<div className="lg:col-span-4">
								...
								<select
									...
									className="w-full h-9 rounded-md bg-gray-50 border border-gray-100 text-[10px] font-semibold focus:ring-red-500 focus:border-red-500 transition"
								>
							...
							<div className="lg:col-span-3">
								...
								<input
									...
									placeholder="0.00"
									type="number"
									className="w-full h-9 rounded-md bg-gray-50 border border-gray-100 text-[10px] font-black focus:ring-red-500 focus:border-red-500 transition disabled:opacity-50 px-3"
								/>
							</div>
							...
							<div className="lg:col-span-3">
								...
								<button
									...
									className="w-full h-9 bg-red-800 border border-red-800 text-white rounded-md text-[10px] font-bold hover:bg-red-900 disabled:bg-gray-100 disabled:text-gray-400 transition cursor-pointer"
								>
					...
					<div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
						...
						<div className="relative bg-white w-full max-w-lg rounded-md p-5 overflow-hidden">
							...
							<div className="grid grid-cols-2 gap-2">
								<a
									href={receiptData.receipt_pdf_url || receiptData.receipt_url}
									target="_blank"
									rel="noreferrer"
								...
								<a
									href={receiptData.receipt_download_url || receiptData.receipt_url}
									target="_blank"
									rel="noreferrer"
									className="h-10 rounded-md bg-slate-700 text-white text-xs font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition"
								>
								...
								<button
									onClick={() => {
										...
										const recent = Array.isArray(receiptData.recent_payments) && receiptData.recent_payments.length
											...
											: "";
										const message = `Payment received. Receipt ${receiptData.receipt_no}. Paid: Rs ${receiptData.amount}. Remaining: Rs ${receiptData.due_amount ?? 0}. Open receipt: ${receiptData.receipt_url}.${recent}`;
										const url = `https://wa.me/${fullNumber}?text=${encodeURIComponent(message)}`;
										window.open(url, "_blank", "noopener,noreferrer");
									}}
									disabled={!(receiptData.user_phone || user.phone)}
									className="col-span-2 h-10 rounded-md bg-emerald-600 text-white text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-700 transition disabled:opacity-60"
								>
...
function Info({ label, value, icon }) {
	return (
		<div className="flex gap-2">
			<div className="w-7 h-7 rounded-md bg-white border border-gray-100 flex items-center justify-center text-gray-400 shrink-0">
...
function StatusBadge({ status }) {
	if (status === 'completed') {
		return (
			<span className="px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded-md bg-emerald-100 text-emerald-700 flex items-center gap-1 shrink-0">
				<span className="w-1 h-1 rounded-md bg-emerald-700 animate-pulse"></span>
				Done
			</span>
		);
	}
	return (
		<span className="px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded-md bg-red-100 text-red-700 flex items-center gap-1 shrink-0">
			<span className="w-1 h-1 rounded-md bg-red-700 animate-pulse"></span>
			Due
		</span>
	);
}
...
function EditUserModal({ user, onClose }) {
	...
	return (
		<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
			<div className="bg-white rounded-md w-full max-w-xl max-h-[90vh] overflow-y-auto p-4">
